...
fn make_libsodium(target: &str, source_dir: &Path, install_dir: &Path) -> PathBuf {
    ...
    let make_arg = if cross_compiling || target.contains("-linux-ohos") { "all" } else { "check" };
...
/// Patch config.sub to recognize OHOS targets.
///
/// The upstream config.sub (timestamped 2020-12-22) does not recognize
/// `*-linux-ohos` as a valid OS.  We prepend a compat shim that rewrites
/// `*-linux-ohos` → `*-linux-gnu` so that configure proceeds, while the
/// actual compilation still uses the OHOS clang toolchain and sysroot.
fn patch_config_sub(source_dir: &Path) {
    use std::io::{BufRead, BufReader};

    let config_sub = source_dir.join("build-aux").join("config.sub");
    if !config_sub.exists() {
        return;
    }

    let marker = "OHOS compatibility patch for librustdesk_core";
    let content = fs::read_to_string(&config_sub).unwrap_or_default();
    if content.contains(marker) {
        return;
    }

    let file = fs::File::open(&config_sub).expect("Failed to open config.sub");
    ...
    for (idx, line) in reader.lines().enumerate() {
        ...
        if idx == 0 && !inserted {
            patched.push("# OHOS compatibility patch for librustdesk_core".to_string());
            patched.push("case \"\\$1\" in".to_string());
            patched.push("  *-linux-ohos)".to_string());
            patched.push("    set -- \"\\${1%-linux-ohos}-linux-gnu\"".to_string());
            patched.push("    ;;".to_string());
            patched.push("esac".to_string());
            inserted = true;
        }
    }

    // Fallback: if we didn't insert (e.g. empty file), prepend
    if !inserted {
        let mut pre = Vec::new();
        pre.push("# OHOS compatibility patch for librustdesk_core".to_string());
        pre.push("case \"\\$1\" in".to_string());
        pre.push("  *-linux-ohos)".to_string());
        pre.push("    set -- \"\\${1%-linux-ohos}-linux-gnu\"".to_string());
        pre.push("    ;;".to_string());
    ...
    let new_content = patched.join("\n") + "\n";
    fs::write(&config_sub, new_content).expect("Failed to write patched config.sub");
}
...
fn build_libsodium() {
    ...
    // Patch config.sub to recognize OHOS host triples
